...
def api_binding_generator(header_path: str, header_excludes: list, js_out_path: str, cpp_out_path: str, api_name: str, namespace: str):
    ...
    if not cpp_out_path.endswith('.cpp'):
        cpp_out_path += f'/{api_name}_export.cpp'
    ...
    with open(js_out_path, 'w') as js_file:
        ...
        content += "}\n"
        # 将新内容写回文件
        with open(cpp_out_path, 'w') as file:
            file.write(content)
        if len(namespace) > 0:
            js_file.write('};\n')
